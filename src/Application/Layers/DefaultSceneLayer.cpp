...
#pragma endregion 
		...
		Material::Sptr megamanMaterial = ResourceManager::CreateAsset<Material>(deferredForward);
		{
			megamanMaterial->Name = "MegaMan";
			megamanMaterial->Set("u_Material.AlbedoMap", megamanTex);
			megamanMaterial->Set("u_Material.NormalMap", normalMapDefault);
			megamanMaterial->Set("u_Material.Shininess", 0.1f);
		}
		...
		{
			camera->SetPostion({ 0.8f, 11.2f, 2.4f });
			camera->SetRotation(glm::vec3(75.0f, 0.0f, -178.5f));
			camera->SetScale(glm::vec3(0.6f, 1.0f, 1.0f));
		...
		GameObject::Sptr megamanBox = scene->CreateGameObject("Megaman");
		{
		...
		GameObject::Sptr monkey1 = scene->CreateGameObject("Monkey 1");
		{
			...
			GameObject::Sptr particles1 = scene->CreateGameObject("Particles");
			monkey1->AddChild(particles1);
			particles1->SetPostion({ 1.5f, 0.0f, 1.0f });
			ParticleSystem::Sptr particleManager1 = particles1->Add<ParticleSystem>();
			particleManager1->Atlas = particleTex;
			...
			emitterr2.TexID = 2;
			emitterr2.Position = glm::vec3(0.0f);
			emitterr2.Color = glm::vec4(1.0f, 0.5f, 0.0f, 1.0f);
			emitterr2.Lifetime = 1.0f / 40.0f;
			emitterr2.SphereEmitterData.Timer = 1.0f / 40.0f;
			emitterr2.SphereEmitterData.Velocity = 0.1f;
			emitterr2.SphereEmitterData.LifeRange = { 0.5f, 1.0f };
		...
		GameObject::Sptr shadowCaster = scene->CreateGameObject("Othro Shadow Light");
		{
			// Set position in the scene
			shadowCaster->SetPostion(glm::vec3(-14.1f, -16.4f, 5.0f));
			shadowCaster->SetRotation(glm::vec3(50.3f, 0.0f, 0.0f));
			shadowCaster->SetScale(glm::vec3(0.8f, 1.0f, 2.5f));

			// Create and attach a renderer for the monkey
			ShadowCamera::Sptr shadowCam = shadowCaster->Add<ShadowCamera>();
			shadowCam->SetProjection(glm::ortho(0.0f, 30.0f, 20.0f, 0.0f, 0.0f, 15.0f));
		}
